...
const educacion= ref([]);
educacion.value= [
    {fecha:"Diciembre/2024", titulo:"Técnico Universitario en Programación", descripcion:"Fin del cursado del segundo semestre", enlace: ""},
    {fecha:"Julio/2024", titulo:"Técnico Universitario en Programación", descripcion:"Fin del cursado del primer semestre", enlace: ""},
    {fecha:"Marzo/2024", titulo:"Técnico Universitario en Programación", descripcion:"Inicio del cursado del primer semestre", enlace: ""},
];
</script>
...
<template>
    ...
    <ul>
        ...
        <!--Aqui vemos con el uso de v-bind (:) que bindeamos el atributo href de html con el item.enlace-->
...
body {
    ...
    align-content: center; /*Centra verticalmente el codigo dentro de la cuadricula*/
    gap: 2rem; /*Espacios entre los elementos de la cuadricula*/
    padding: 2rem; /*Espaciado interno dentro de la cuerpo*/
...
ul {
    ...
    --line-w: 0.25rem; /*Ancho de la linea que conecta los elementos de las lista*/
...
ul::before {
    ...
    background-color: rgba(225, 225, 225); /*Color de la linea*/
    border-radius: calc(var(--line-w) / 2); /*Bordes redondeados para la linea*/
/*Elementos para los elementos de la lista que no son el ultimo*/
}
...
ul li .fecha {
    ...
    text-align: center;
    background-color: var(--fecha-color); /*Color de fodo, usando una variable personalizada*/
    color: white;
    font-size: 1.25rem;
    font-weight: 700; /*Hace el texto en negrita*/
    display: grid; /*Usa el layout de cuadricula*/
    place-content: center; /*Centra el contenido de la fecha*/
    position: relative; /*Posiciona la fecha relativa a su contenedor*/
    /*border-radius: calc(var(--dateH) / 3) 0 0 calc(var(--dateH)) / 2; Bordes redondeados para la fecha*/
}
...
ul li .fecha::before {
    ...
    aspect-ratio: 1; /*Mantiene una relacon aspecto 1:1 para crear un cuadrado*/
    background-color: var(--fecha-color); /*Usa el color de fondo de la fecha*/
...
/*Estilo para el circula que conecta la fecha con  la linea*/
ul li .fecha::after {
    ...
    aspect-ratio: 1; /*Mantiene una relacion de aspecto 1:1 para el circulo*/
    ...
    top: 50%; /*Cemtra verticalmente el circulo dentro del contenedor de la fecha*/
...
ul li .descripcion::before {
    ...
    height: 0.5rem; /*Altura de la sombre*/
    background: rgba(0, 0, 0, 0.5); /*Color de fondo oscuro para simular la sombra*/
...
@media (min-width: 40rem) {
    ...
    ul li:nth-child(odd) .fecha::before {
        clip-path: polygon(0 0, 100% 0, 100% 100%); /*Coloca los items impares en la primera columna*/
    }
